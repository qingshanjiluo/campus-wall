...
const poolSchema = z.object({
  ...
  min_choice: z.coerce.number<number>().int().min(1).default(1),
  max_choice: z.coerce.number<number>().int().min(1).default(1),
...
export const createPollSchema = poolSchema.extend({
...
export const updatePollSchema = poolSchema.extend({
  ...
  options: z.object({
    ...
    update: z
      .array(
        z.object({
          option_id: z.coerce.number<number>().min(1).max(9999999),
          text: z
            .string()
            .min(1, '选项内容不能为空')
